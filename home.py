...
`st.container` now works like a simplified flexbox:
...
with tab3:
    with st.echo():
        with st.container(gap="large", horizontal_alignment="right"):
...
column_config = {
    ...
    "Last Updated": st.column_config.DatetimeColumn(format="MMM DD, YYYY h:mm a"),
...
"""
Use cases:
- Show details for a data point
- Run analysis on a subset of the data
- Cross-filter dataframes and charts

## Fragments :blue-badge[🏃‍♀️ Performance]

Fragments allow you to rerun only a part of your app when interacting with a widget:
"""
...
if st.toggle("Show fragment example (slow)", False):
    ...
    if st.toggle("Wrap in fragment", False):
        st.code("""@st.fragment
def say_hello():
    if st.button("Say hello"):
        st.write(np.random.choice(greetings))  
...
with st.echo():
    if not st.user.is_logged_in:
        ...
        if st.button("Logout"):
            st.logout()
        st.write(f"Hello, {st.user.name}! 👋 Your email is {st.user.email}.")
